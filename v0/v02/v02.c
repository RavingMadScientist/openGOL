...
v0.2
Adding gen and live counters and printing in zoom-independent location
...
//    glRasterPos3f(5.0, 5.0, 0.0); 
    char * d;
    char* msg = "Hello There in the Game of Life";
    void* font = "GLUT_STROKE_ROMAN";
    int counter = 0;
    for (d = msg; *d != '\0'; d++){
        printf("stroking character %c\n", *d);
//        glutStrokeCharacter(GLUT_STROKE_ROMAN, *d);
        counter ++;
        glRasterPos2f(counter, counter);
        glDrawArrays(GL_TRIANGLE_STRIP, 0 , 6);
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_10, *d);
        }

    glColor3f(0.0, 1.0, 1.0);    
 //   glRasterPos3f(200.0, 200.0, 0.0); 
    for (d = msg; *d != '\0'; d++){
        printf("stroking character %c\n", *d);
        counter ++;
        glRasterPos2f(2*counter, 2*counter);
//        glutStrokeCharacter(GLUT_STROKE_ROMAN, *d);
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_10, *d);        
        }
    ...
    for (d = msg; *d != '\0'; d++){
        printf("stroking character %c\n", *d);
        counter ++;
        glRasterPos2f(10*counter, 10*counter);
//        glutStrokeCharacter(GLUT_STROKE_ROMAN, *d);
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_10, *d);    
        }
