...
//    char* msgfrag = "Hello World row";
//    char* msgfrag[msgRows] = {"Generation: ", "Orgs: ", "Live Pct: ", "abcdefghjklmostuvwxyz" };
    char localmsg[msgLength];
    int len; 
    //char val[1];
    for (row=0; row<msgRows; row++){
        //strcpy(localmsg, msgfrag);
        //val[0] = (char)row;
        //strcat(localmsg, val);
...
//                }
         ...
         //       printf ("orgCenter[%d]=(%f,%f,%f); r=%f\n", i, oCenters[3*i],oCenters[3*i+1],oCenters[3*i+2], rPointer[i]);
         //       if (i == 0){                
          //      printf("Vertices:");
         //       for (j=0; j<4; j++){
         //           printf("(");
         //           for (k=0; k<4; k++){
        //                printf("%f,",cPointer[16*i+4*j+k]);
        //                }
        //            printf(")\n");                
         //           }
         //           }
...
//                int w;
//                for(w=0; w<16; w++){
//                    printf("f-update conclusion: cPointer[%d]=%f\n",w, cPointer[w]);                
//                    }   
            glutPostRedisplay();     
            break;
            }
