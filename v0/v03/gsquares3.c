...
static unsigned char chessboard2[64][64][4];
...
const GLchar* FragmentShader = 
{
    ...
    "{\n"\
    "   \\out_Color = ex_Color;\n"\
    "   out_Color = texture(aTexture, TexCoord;\n"\
    "}\n"
};
...
void createChessboard2(void){
   int i, j;
   for (i = 0; i < 64; i++) 
      for (j = 0; j < 64; j++) 
         if ( ( ((i/8)%2) && ((j/8)%2) ) || ( !((i/8)%2) && !((j/8)%2) ) )
		 {
            chessboard2[i][j][0] = 0x00;
            chessboard2[i][j][1] = 0x00;
            chessboard2[i][j][2] = 0x00;
            chessboard2[i][j][3] = 0xFF;
		 }
		 else
		 {
            chessboard2[i][j][0] = 0x00;
            chessboard2[i][j][1] = 0xFF;
            chessboard2[i][j][2] = 0x00;
            chessboard2[i][j][3] = 0xFF;
		 }
    }
...
void CreateProcTexture(){
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 64, 64, 0, GL_RGBA, GL_UNSIGNED_BYTE, chessboard2);
...
//texture Coords
    ...
    glBindVertexArray(VaoId[0]);
    printf("VaoId[0] bound\n");
    glGenBuffers(3, VboId);
    glBindBuffer(GL_ARRAY_BUFFER, VboId[0]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(triVertices), triVertices, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(0);    
    printf("TriVertices bound\n");
...
void CreateShaders(void) {
    ...
    //textureLocation = glGetUniformLocation(ProgramId, "aTexture");
